...
function renderCart() {
  ...
  arrSetUnique.forEach((item) => {
    ...
    let data = {
      maKH: kh.maKH,
      maPB: item.memory.maPB,
      soLuong: count,
      donGia: item.price * count,
    };
    console.log("data: ", data);

    // Gọi API để lưu dữ liệu giỏ hàng vào cơ sở dữ liệu
    $.ajax({
      url: "http://localhost:8080/phonestore/save-cart", // Địa chỉ API của bạn
      type: "POST",
      contentType: "application/json",
      data: JSON.stringify(data), // Chuyển dữ liệu thành định dạng JSON
      success: function (response) {
        console.log("Dữ liệu đã được lưu:", response);
        // Bạn có thể xử lý phản hồi từ server nếu cần
      },
      error: function (error) {
        console.log("Có lỗi khi gửi dữ liệu:", error);
      },
    });
