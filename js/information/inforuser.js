...
async function fetchUserData() {
  try {
    const response = await fetch(
      `http://localhost:8080/user/get-user?id=${user.tkId}`
    );
    ...
    if (gioiTinh) {
      const genderRadioButton = document.querySelector(
        `input[name="gender"][value="${gioiTinh}"]`
      );
...
async function updateCustomerInfo() {
  try {
    ...
    const updateTaiKhoanResponse = await fetch(
      "http://localhost:8080/user/update-taikhoan",
      {
        method: "POST",
    ...
    const updateKhachHangResponse = await fetch(
      "http://localhost:8080/user/update-khachhang",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
