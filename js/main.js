    ...
    function loadHeader() {
        $('#header').load('/html/header/header.html', () => {
            ...
            if (kh) {
                ...
                $('#dangnhapTK').text(kh.hoTen).attr('href','/html/home/thongtinuser.html');
                
                
                $('#dangNhap').hide();
                $('#tenTk').show();    
            } else {
                $('#dangnhapTK').text('Đăng nhập').attr('href','/html/home/login.html'); 
...
async function renderProduct() {
    try {
        ...
        products.forEach((item) => {
            ...
            const memoryButtons = (item.memory || []).map(mmr => `<b class="btn btn-default btn-sm">${mmr}</b>`).join(' ');

            // Tạo nút riêng cho mỗi ROM từ phienbansps
            const romButtons = item.phienbansps.map(pb => `<b class="btn btn-default btn-sm">${pb.ROM}GB</b>`).join(' ');

            // Lấy giá cũ và giá bán từ phiên bản sản phẩm đầu tiên (giả sử)
            const oldPrice = convertToVND(item.phienbansps[0].giaBan) || 'Giá cũ không có';
            const newPrice = convertToVND(item.phienbansps[0].giaGiam) || 'Giá bán không có';
