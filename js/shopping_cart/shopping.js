...
function deleteItem(maSP, price) {
    ...
    let indexRM = carts.findIndex((item) => item.maSP == maSP && item.price == price)
...
function removeAll(maSP, price) {
    if(confirm('Bạn chắc chắn muốn bỏ sản phẩm này')) {
        ...
        carts = carts.filter(item => item.maSP !== maSP ||  (item.maSP === maSP && item.price !== price))
        localStorage.setItem('listCart', JSON.stringify(carts));
        renderCart()
    }
}
...
function renderCart() {
    ...
    arrSetUnique.forEach(item => {
        ...
        $('.payment__tableBody').append(
            `
            <tr>
                <td><a onclick="removeAll('${item.maSP}',${item.price}); return false" href="#"><i class="fa fa-times" aria-hmaSPden="true"></i></a></td>
                ...
                <td class="nav__pc">
                    <div class="payment__tableBody--count">
                        <i onclick="deleteItem('${item.maSP}',${item.price})" class="fa fa-minus count_minus" aria-hmaSPden="true"></i>
                        <span>${count}</span>
                        <i onclick="addItem('${item.maSP}',${item.price})" class="fa fa-plus count_plus" aria-hmaSPden="true"></i>
                    </div>
                </td>
