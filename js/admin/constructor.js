...
class Product {
  ...
  static loadProducts(list) {
    ...
    if (localStorage.listProducts) return true;
    return false;
  }
  // Lấy ra 1 sản phẩm
...
class User {
  ...
  constructor(
    ...
    this.userID =
      User.getUsers() === null ? ++User.userID : User.getLastUserID() + 1;
...
class cart {
  ...
  static getCartID(userID, cartID) {
    ...
    let myCart = null;
    list.forEach((item) => {
      if (item.cartID === cartID) myCart = item;
    });
    return myCart;
  }
...
class Invoice {
  ...
  constructor(cartList, userID, orderTime) {
    this.invoiceID =
      Invoice.getInvoices() === null
        ? ++Invoice.total
        : Invoice.getLastInvoiceID() + 1;
  ...
  static loadInvoices(list) {
    ...
    if (localStorage.listInvoices) return true;
    return false;
  }
  ...
  static getTotalPriceOfInvoice(invoiceID) {
    ...
    const sum = invoice.cartList.reduce(
  ...
  static getInvoiceByDateTime(startDate, endDate) {
    ...
    list.forEach((invoice) => {
      ...
      if (
        time.parseDateTime(startDate) <= currentDate &&
        currentDate <= time.parseDateTime(endDate)
      ) {
        console.log(invoice);
        myList.push(invoice);
      }
  ...
  static updateInvoiceStatus(invoiceID, status) {
    ...
    list.forEach((invoice) => {
      if (invoice.invoiceID === invoiceID) {
        ...
        if (
          invoice.userProfile.phone === "" ||
          invoice.userProfile.address === undefined
        ) {
          alert(
            "Khách hàng chưa cung cấp thông tin địa chỉ và số điện thoại giao hàng"
          );
